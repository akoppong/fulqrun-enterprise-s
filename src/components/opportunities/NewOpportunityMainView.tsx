...
import { Opportunity, Company, User, PEAK_STAGES } from '@/lib/types';
...
export function NewOpportunityMainView({ 
  ...
  }, [validCompanies, validContacts, validUsers, setCompanies, setContacts, setAllUsers, user]);

  // Filter contacts by selected company
  useEffect(() => {
    if (formData.company) {
      const filtered = validContacts.filter(contact => contact.company === formData.company);
      setAvailableContacts(filtered);
    } else {
      setAvailableContacts([]);
    }
  }, [formData.company, validContacts]);
  ...
  const handleCompanyChange = (companyName: string) => {
    ...
    const company = validCompanies.find(c => c.name === companyName);
