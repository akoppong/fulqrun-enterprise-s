...
export function MEDDPICCModule() {
  const [assessment, setAssessment] = useKV<MEDDPICCAssessment>('meddpicc-assessment', {
    answers: [],
    pillar_scores: {},
    total_score: 0,
  ...
  useEffect(() => {
    ...
    meddpiccData.pillars.forEach(pillar => {
      ...
      const pillarScore = pillarAnswers.reduce((sum, answer) => sum + answer.score, 0);
  ...
  }, [assessment.answers, setAssessment]);

  const handleAnswerChange = (pillar: string, questionId: string, value: string) => {
  ...
  const generateCoachingPrompts = () => {
    ...
    meddpiccData.pillars.forEach(pillar => {
      ...
      if (pillarScore < maxPillarScore * 0.5) { // Less than 50% of max score
        const prompt = meddpiccData.coaching_prompts.find(cp => cp.condition.pillar === pillar.id);
        if (prompt) {
          prompts.push(`${pillar.title}: ${prompt.prompt}`);
        }
      }
    });
